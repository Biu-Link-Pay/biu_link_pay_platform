...
import type { QueryRecipientModel } from '@/api/card'
import type { FiatPaymentMethodItem, FiatPaymentMethodsModel, FiatRateModel } from '@/api/order'
...
function toggleGroup(methodType: string) {
  expandedGroups.value[methodType] = !expandedGroups.value[methodType]
}

// Initialize payment method fields
function initializePaymentMethodFields(paymentMethod: FiatPaymentMethodItem) {
...
function validatePaymentMethodFields(): boolean {
  ...
  selectedPaymentMethod.value.fields.forEach((field) => {
    ...
    if (field.minLength && valueStr !== '') {
      const minLength = Number.parseInt(field.minLength)
    ...
    if (field.maxLength && valueStr !== '') {
      const maxLength = Number.parseInt(field.maxLength)
...
function selectPaymentMethod(paymentMethod: FiatPaymentMethodItem) {
...
function handleDeleteRecipient() {
  ...
  confirm.require({
    message: t('withdraw.deleteRecipientConfirm'),
    header: t('withdraw.deleteRecipientTitle'),
...
async function handleRecipientSaved() {
  ...
  <div>
    ...
    <div class="hidden md:block space-y-8">
      ...
      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-lg p-8">
        ...
        <div class="space-y-6">
          ...
          <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6">
            ...
            <div class="flex items-center justify-between">
              <div class="text-2xl font-semibold text-gray-900 dark:text-white">
                ...
                >
                  ({{ t('withdraw.usePtsInline', { n: appliedRewardPoints.toLocaleString() }) }})
                </span>
              </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6">
            <div class="flex items-center space-x-2 mb-4">
              <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('withdraw.chooseFiatCurrency') }}</label>
            </div>
            <div class="flex flex-col md:flex-row md:items-center md:space-x-3 space-y-3 md:space-y-0">
              <Dropdown
                v-model="selectedFiatCurrency" :options="fiatCurrencyOptions" option-label="label" disabled
          ...
          <div>
            ...
            >
              <div class="flex items-start justify-between">
                ...
                <div class="flex items-center space-x-2 ml-4">
                  <Button
                    :label="t('common.delete')" icon="pi pi-trash" severity="danger" size="small"
            ...
            <div
              v-else-if="!recipientLoading && !recipientInfo"
              class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6 border border-gray-200 dark:border-gray-600"
            >
              <div class="flex flex-col items-center justify-center py-8">
                <i class="pi pi-user-plus text-4xl text-gray-400 dark:text-gray-500 mb-4" />
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  {{ t('withdraw.noRecipientFound') }}
                </p>
                <Button :label="t('withdraw.addRecipient')" icon="pi pi-plus" @click="showRecipientDialog = true" />
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6">
            ...
            <div v-else class="space-y-4">
              <div
                v-for="group in paymentMethodGroups" :key="group.methodType"
                class="border border-gray-200 dark:border-gray-600 rounded-xl overflow-hidden"
              >
                <!-- Group Header -->
                <div
                  class="bg-gray-100 dark:bg-gray-700 px-5 py-3 flex items-center justify-between cursor-pointer"
                  @click="toggleGroup(group.methodType)"
                >
                  <div class="flex items-center space-x-3">
                    <i
                      class="pi pi-chevron-down text-gray-600 dark:text-gray-300 transition-transform duration-200"
                      :class="{ 'rotate-180': expandedGroups[group.methodType] }"
                    />
                    <h4 class="text-base font-semibold text-gray-900 dark:text-white">
                ...
                <div v-show="expandedGroups[group.methodType]" class="p-4 space-y-3">
                  <div
                    v-for="paymentMethod in group.fiatPaymentMethod" :key="paymentMethod.methodCode"
                  ...
                  >
                    <div class="flex flex-col gap-3">
                      <div class="flex items-center space-x-4">
                        ...
                        >
                          <img
                            v-if="paymentMethod.logoUrl" :src="paymentMethod.logoUrl" :alt="paymentMethod.methodName"
                        ...
                        <div
                          v-if="selectedPaymentMethod?.methodCode === paymentMethod.methodCode"
          ...
          <div
            v-if="selectedPaymentMethod && !isReceiveAmountWithinLimit"
          ...
          <div
            v-if="selectedPaymentMethod && selectedPaymentMethod.fields && selectedPaymentMethod.fields.length > 0"
            class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6 border border-gray-200 dark:border-gray-600"
          >
            <div class="flex items-center space-x-2 mb-4">
              <i class="pi pi-cog text-blue-600 dark:text-blue-400" />
              <h4 class="text-base font-semibold text-gray-900 dark:text-white">
                {{ t('withdraw.paymentMethodDetails') }}
              </h4>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-for="field in selectedPaymentMethod.fields" :key="field.name">
                ...
                <!-- Text Input -->
                <InputText
                  v-if="field.type === 'text'" v-model="paymentMethodFields[field.name]"
                ...
                <InputText
                  v-else v-model="paymentMethodFields[field.name]" :placeholder="field.placeholder"
                  :maxlength="field.maxLength ? parseInt(field.maxLength) : undefined"
    ...
    <div class="md:hidden space-y-4">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <Dropdown
          v-model="selectedFiatCurrency" :options="fiatCurrencyOptions" option-label="label" disabled
          option-value="value" :placeholder="t('withdraw.selectCurrency')" class="w-full" filter show-clear
        />
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <!-- Loading State -->
        <div v-if="recipientLoading" class="flex items-center justify-center py-8">
          <i class="pi pi-spin pi-spinner text-xl text-blue-600 dark:text-blue-400" />
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center space-x-2 mb-3">
                <i class="pi pi-user text-blue-600 dark:text-blue-400 text-sm" />
            ...
            <div class="flex flex-col space-y-2 ml-3">
              <Button
                :label="t('common.delete')" icon="pi pi-trash" severity="danger" size="small" class="text-xs"
                @click="handleDeleteRecipient"
              />
            </div>
          </div>
        ...
        >
          <div class="flex flex-col items-center justify-center py-6">
            <i class="pi pi-user-plus text-3xl text-gray-400 dark:text-gray-500 mb-3" />
            <p class="text-xs text-gray-600 dark:text-gray-400 mb-3">
              No recipient information found
            </p>
            <Button :label="t('withdraw.addRecipient')" icon="pi pi-plus" size="small" @click="showRecipientDialog = true" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <div v-if="selectedPaymentMethod" class="w-4 h-4 bg-blue-600 rounded-full" />
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            <!-- Group Header -->
            <div
              class="bg-gray-100 dark:bg-gray-700 px-4 py-2.5 flex items-center justify-between cursor-pointer"
              @click="toggleGroup(group.methodType)"
            >
              <div class="flex items-center space-x-2">
                <i
                  class="pi pi-chevron-down text-gray-600 dark:text-gray-300 text-xs transition-transform duration-200"
                  :class="{ 'rotate-180': expandedGroups[group.methodType] }"
                />
                <h4 class="text-sm font-semibold text-gray-900 dark:text-white">
                  {{ formatMethodType(group.methodType) }}
      ...
      >
        <div class="flex items-center space-x-2 mb-3">
          <i class="pi pi-cog text-blue-600 dark:text-blue-400 text-sm" />
          <h4 class="text-sm font-semibold text-gray-900 dark:text-white">
            {{ t('withdraw.paymentMethodDetails') }}
          </h4>
        </div>
