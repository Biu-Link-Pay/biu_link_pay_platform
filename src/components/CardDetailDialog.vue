...
import type { CardDetailResponse, CardHolderInfo as HolderInfo } from '@/api/card'
...
async function refreshDisplayNames() {
  ...
  if (countryCode) {
    if (countries.value.length === 0) {
      try { countries.value = await getCountries() }
      catch { }
    }
...
function loadCitiesForState(countryCode?: string | null, stateIsoCode?: string | null, preserveExisting = false) {
  ...
  getCitiesOfState(countryCode || '', stateIsoCode || '').then((list) => {
    ...
    cities.value = cityList.map((c: CityOption) => ({ name: c.name }))
    if (preserveExisting && form.residentialCity) {
      const matched = cityList.find((c: CityOption) => c.name === form.residentialCity)
      if (matched)
        selectedCityName.value = matched.name
    }
  })
}

function loadCitiesForCountry(countryCode?: string | null, preserveExisting = false) {
  if (!countryCode) {
    cities.value = []
    selectedCityName.value = null
    if (!preserveExisting)
...
function loadStatesForCountry(countryCode?: string | null, preserveExisting = false) {
  if (!countryCode) {
    states.value = []
  ...
  getStatesOfCountry(countryCode || '').then((stateList) => {
    ...
    if (preserveExisting && form.residentialState) {
      ...
      if (matched)
        selectedStateCode.value = matched.isoCode
    }
  })
}
...
async function saveAddress() {
  ...
  try {
    ...
    if (resp && (resp as any).success) {
      toast.add({ severity: 'success', summary: t('cardDetail.saved'), detail: t('cardHolder.addressSaved'), life: 3000 })
      ...
      toast.add({ severity: 'error', summary: t('cardDetail.saveFailed'), detail: (resp as any)?.msg || t('cardHolder.saveFailed'), life: 3000 })
    }
  }
  catch (error) {
    console.error('Failed to save billing address:', error)
    toast.add({ severity: 'error', summary: t('common.error'), detail: (error as any)?.message || t('common.unexpectedError'), life: 3000 })
  }
  finally {
    saving.value = false
  }
...
<template>
  <Dialog
    :visible="visible" modal :header="t('cardDetail.title')" :style="{ width: '90vw', maxWidth: '600px' }"
    :breakpoints="{ '960px': '75vw', '640px': '95vw' }" :draggable="false" class="card-detail-dialog"
    @update:visible="onUpdateVisible"
  >
    <div v-if="loading" class="flex justify-center py-12">
      <div class="text-center">
        <i class="pi pi-spin pi-spinner text-3xl text-blue-600 mb-3" />
    ...
    <div v-else-if="cardDetail" class="space-y-6">
      ...
      >
        <div class="flex items-center justify-between mb-3">
          ...
          <button
            ...
            :title="t('cardDetail.copyCardNumber')"
            @click="() => { copyToClipboard(formatCardNumber(cardDetail?.cardNo)); toast.add({ severity: 'success', summary: t('common.success'), detail: t('common.cardNumberCopied'), life: 2000 }) }"
          >
            <i class="pi pi-copy text-blue-600 dark:text-blue-400 text-sm" />
          </button>
        </div>
      ...
      <div v-if="hasBillingAddress" class="bg-gray-50 dark:bg-gray-800/60 rounded-xl p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 tracking-wide">
            {{ t('cardDetail.billingAddress') }}
          </div>
          <div class="flex items-center gap-2">
            <button
              class="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors"
              :title="t('cardDetail.copyAddress')" @click="() => { copyToClipboard(`${localDetail?.billingAddress || ''}, ${localDetail?.billingCity || ''}, ${localDetail?.billingState || ''} ${localDetail?.billingPostalCode || ''}, ${localDetail?.billingCountry || ''}`); toast.add({ severity: 'success', summary: t('common.success'), detail: t('common.addressCopied'), life: 2000 }) }"
            >
              <i class="pi pi-copy text-gray-600 dark:text-gray-300 text-sm" />
            </button>
            <Button :label="t('cardDetail.edit')" icon="pi pi-pencil" size="small" @click="startEdit" />
          </div>
        </div>

        <!-- Readonly view -->
        <div v-if="!isEditing" class="space-y-1 text-sm text-gray-700 dark:text-gray-300">
          <div class="font-medium break-all whitespace-pre-wrap">
            {{ localDetail?.billingAddress }}
          </div>
          <div>
            {{ localDetail?.billingCity }}, {{ displayState || getStateDisplay(localDetail?.billingCountry,
        ...
        <div v-else class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('cardDetail.country') }}</label>
            <Dropdown
              v-model="form.residentialCountryCode" :options="countries" option-label="name" option-value="code"
              :placeholder="t('cardDetail.selectCountry')" class="w-full" filter show-clear
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('cardDetail.stateProvince') }}</label>
              <Dropdown
                v-if="hasStateOptions" v-model="selectedStateCode" :options="states" option-label="name"
                option-value="isoCode" :placeholder="t('cardDetail.selectState')" class="w-full" filter show-clear
                :class="{ 'p-invalid': errors.residentialState }"
              />
              <InputText
                v-else v-model="form.residentialState" :placeholder="t('cardDetail.enterState')" class="w-full"
                :class="{ 'p-invalid': errors.residentialState }"
              />
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('cardDetail.city') }}</label>
              <Dropdown
                v-if="hasCityOptions" v-model="selectedCityName" :options="cities" option-label="name"
                option-value="name" :placeholder="t('cardDetail.selectCity')" class="w-full" filter show-clear
                :class="{ 'p-invalid': errors.residentialCity }"
              />
              <InputText
                v-else v-model="form.residentialCity" :placeholder="t('cardDetail.enterCity')" class="w-full"
                :class="{ 'p-invalid': errors.residentialCity }"
              />
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('cardDetail.address') }}</label>
            <InputText
              v-model="form.residentialAddress" :placeholder="t('cardDetail.enterAddress')" class="w-full"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('cardDetail.postalCode') }}</label>
            <InputText
              v-model="form.residentialPostalCode" :placeholder="t('cardDetail.enterPostalCode')" class="w-full"
          ...
          <div class="mt-2 flex gap-3">
            <Button :label="t('common.cancel')" icon="pi pi-times" severity="secondary" class="flex-1" @click="cancelEdit" />
            <Button :label="t('cardDetail.save')" icon="pi pi-check" class="flex-1" :loading="saving" @click="saveAddress" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4 mt-6">
        <div class="flex items-center space-x-3">
          ...
          >
            <i class="pi pi-shield text-amber-600 dark:text-amber-400 text-sm" />
          </div>
          <div>
            <p class="text-sm font-medium text-amber-800 dark:text-amber-200">
              {{ t('cardDetail.securityReminder') }}
            </p>
          </div>
        </div>
      </div>
