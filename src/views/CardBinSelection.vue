...
import type { CardBin, CardConfig } from '@/api/card'
...
const selectedBinInfo = computed<CardBin | null>(() => {
  if (!filteredCardBins.value.length)
    return null
...
function filterByScheme(scheme: string | null) {
...
// formatRechargeFee
function formatRechargeFee(amount: number) {
  return `${amount}%`
}
// Format percentage
function _formatPercentage(percentage: number) {
  return `$${percentage}`
}

// Format card number
function formatCardNumber(cardBin: string | null) {
  if (!cardBin) {
    return '**** **** **** ****'
  }
  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    >
      ...
      <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
        <div class="flex items-center space-x-3">
          <div class="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center flex-shrink-0">
            <i class="pi pi-credit-card text-white text-sm" />
          </div>
          <p class="text-sm text-gray-700 dark:text-gray-300">
            New Virtual Cards Added to Your List
          </p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-3 gap-4 mb-8">
        <div class="text-center">
          <div class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ formatCurrency(selectedCard?.applyFee || 0)
            }}
          </div>
        ...
        <div class="text-center">
          <div class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ formatRechargeFee(selectedCard?.rechargeFee
              || 0) }}
          </div>
          <div class="text-sm text-gray-500 dark:text-gray-400">
            Recharge fee
        ...
        <div class="text-center">
          <div class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ formatCurrency(selectedCard?.monthlyFee || 0)
            }}
          </div>
          <div class="text-sm text-gray-500 dark:text-gray-400">
            Monthly Fee
      ...
      <div class="mb-8">
        ...
        <div v-else-if="binError" class="text-center py-8">
          <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6">
            <i class="pi pi-exclamation-triangle text-red-500 text-2xl mb-3" />
        ...
        <div v-else-if="cardBins.length > 0" class="space-y-4">
          ...
          <div class="md:hidden mt-6 flex flex-col items-center">
            ...
            >
              ...
              <button
                v-for="scheme in cardSchemeOptions" :key="scheme" type="button"
                class="px-3.5 py-1.5 rounded-full border text-xs font-medium flex-shrink-0 transition-colors"
            ...
            <div class="relative w-full max-w-xs sm:max-w-sm mx-auto">
              <div
                class="overflow-hidden" @touchstart="handleTouchStart" @touchmove="handleTouchMove"
              ...
              >
                ...
                >
                  <div v-for="(binInfo, index) in filteredCardBins" :key="index" class="w-full flex-shrink-0 px-2">
                    <div
                      class="rounded-xl p-6 text-white shadow-lg relative overflow-hidden" :style="{
              ...
              <div class="flex justify-center mt-4 space-x-2">
                <div
                  v-for="(binInfo, index) in filteredCardBins" :key="index"
                  class="w-2 h-2 rounded-full transition-colors"
                  :class="currentBinIndex === index ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'"
                />
              </div>
            </div>
          </div>
            ...
            <div class="w-full">
              ...
              <div class="flex items-center justify-between mb-8">
                ...
                <div v-if="cardSchemeOptions.length" class="flex items-center space-x-2">
                  ...
                  <button
                    type="button" class="px-2.5 py-1 rounded-full border text-xs font-medium transition-colors"
                    :class="!activeSchemeFilter
                      ? 'bg-blue-600 text-white border-blue-600'
              ...
              <div class="flex items-center justify-center space-x-8">
                ...
                <button
                  ...
                  title="Previous card" @click="previousBin"
                >
                  <i class="pi pi-chevron-left text-gray-600 dark:text-gray-400 text-lg" />
                </button>

                <!-- Main Content Area -->
                <div class="flex-1 max-w-4xl">
                  <div
                    ref="carouselContainer" class="overflow-hidden rounded-xl" tabindex="0"
                    style="outline: none;" @mousedown="handleMouseDown" @mousemove="handleMouseMove"
                    @mouseup="handleMouseUp" @mouseleave="handleMouseUp" @wheel="handleWheel" @keydown="handleKeyDown"
                  >
                    <!-- Card Carousel -->
                    <div
                      class="flex transition-transform duration-300 ease-in-out"
                ...
                <button
                  :disabled="currentBinIndex === cardBins.length - 1"
                  class="flex-shrink-0 w-12 h-12 bg-white dark:bg-gray-800 rounded-full shadow-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center"
                  title="Next card" @click="nextBin"
                >
                  <i class="pi pi-chevron-right text-gray-600 dark:text-gray-400 text-lg" />
                </button>
              </div>
              ...
              <div class="flex justify-center mt-6 space-x-2">
                <button
                  ...
                  :title="`Go to card ${index + 1}`" @click="currentBinIndex = index"
                />
              </div>
            </div>
          </div>
        ...
        <div v-else class="text-center py-12">
          <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-8">
            <i class="pi pi-credit-card text-gray-400 dark:text-gray-500 text-4xl mb-4" />
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
              No Card BINs Available
            </h3>
            <p class="text-gray-600 dark:text-gray-400">
              No card BIN information found for this card type.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="bottom-buttons-container relative md:static">
        ...
        <!-- Order Button -->
        <button
          :disabled="!selectedBinInfo || binLoading" class="bottom-button-dual bottom-button-dual-primary"
          @click="orderCard"
        >
