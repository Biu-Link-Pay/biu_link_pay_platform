...
import type { OrderCryptoNetwork, OrderPayType } from '@/api/order'
...
const _availableRewardPoints = computed(() => userStore.cardRewardPoints || 0)
const appliedRewardPoints = ref(0)
const discountAmount = ref(0)
const finalPayAmount = ref(payAmount.value)

function recalculateAmountsFromPoints() {
  ...
  discountAmount.value = Number.parseFloat(effectiveDiscount.toFixed(2))
  const final = base - effectiveDiscount
  finalPayAmount.value = final > 0 ? Number.parseFloat(final.toFixed(2)) : 0
}
...
const isAmountWithinLimit = computed(() => {
  ...
  const paymentAmount = finalPayAmount.value
...
function selectPayType(payType: OrderPayType) {
...
function selectCrypto(crypto: OrderCryptoNetwork) {
...
async function queryRate() {
  if (!selectedCrypto.value || !selectedPayType.value || payAmount.value <= 0) {
    rateResult.value = null
    actualCryptoAmount.value = ''
    return
  }
  ...
  try {
    ...
    if (response.success && response.model) {
      ...
      const appliedPoints = Number.isFinite(parsedCardPoints) ? parsedCardPoints : requestedPoints
      ...
      const totalFiat = Number.parseFloat(totalFiatString)
      if (!Number.isNaN(totalFiat)) {
        finalPayAmount.value = totalFiat > 0 ? Number.parseFloat(totalFiat.toFixed(2)) : 0
      }
      else {
        const fallbackAmount = payAmount.value - discountAmount.value
        finalPayAmount.value = fallbackAmount > 0 ? Number.parseFloat(fallbackAmount.toFixed(2)) : 0
      }
    }
    else {
      toast.add({
  ...
  try {
    ...
    if (orderResponse.success && orderResponse.model) {
      ...
      cardStore.setCurrentOrder({
        ...
        amount: finalPayAmount.value.toString(),
    ...
    >
      ...
      <div class="hidden md:block">
        <div class="max-w-4xl xl:max-w-5xl mx-auto space-y-10">
          ...
          >
            <div class="absolute -top-16 -right-20 w-56 h-56 bg-white/15 blur-3xl rounded-full" />
            <div class="absolute -bottom-20 -left-10 w-64 h-64 bg-white/10 blur-3xl rounded-full" />
            <div class="relative px-10 py-12 text-center flex flex-col items-center space-y-4">
              ...
              <div class="text-5xl font-extrabold tracking-tight">
                {{ formatCurrency(originalPayAmount) }}
              </div>
              ...
              >
                <i class="pi pi-shield mr-2 text-white/90" />
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center">
                  <i class="pi pi-credit-card text-blue-600 dark:text-blue-400 text-base" />
                </div>
                <div>
                  ...
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    Choose a provider and network to continue
                  </p>
                </div>
              </div>
                  ...
                  <div class="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div class="text-sm text-gray-500 dark:text-gray-400">
                        {{ payType.cryptoNetworks.length }} crypto
                        options
                      </div>
                    </div>
                  </div>
          ...
          <div
            v-if="selectedCrypto && actualCryptoAmount"
          ...
          <div
            v-if="selectedCrypto && !isAmountWithinLimit"
      ...
      <div class="md:hidden">
        <div class="w-full max-w-md mx-auto space-y-6">
          ...
          >
            ...
            <div
              v-if="rateResult?.cryptoDetail?.cryptoToUsdTRate"
      ...
      <div class="bottom-buttons-container relative md:static">
        ...
        <!-- Continue Button -->
        <button
          :disabled="!selectedPayType || !selectedCrypto || rateLoading || creatingOrder || !isAmountWithinLimit"
