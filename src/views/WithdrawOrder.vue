    ...
    >
      ...
      <div v-if="isDesktopView">
        <div class="max-w-4xl xl:max-w-5xl mx-auto space-y-8">
          ...
          >
            <div class="flex">
              <button
                v-for="tab in tabs" :key="tab.key" class="flex-1 px-6 py-4 text-center text-base font-semibold transition-all duration-200 relative"
          ...
          <CryptoWithdrawal
            v-if="activeTab === 'crypto'" :card-info="cardInfo" :card-detail="cardDetail"
          ...
          <FiatWithdrawal
            v-if="activeTab === 'fiat'" :card-info="cardInfo" :card-detail="cardDetail" :balance="balance"
      ...
      <div v-else class="space-y-4 pb-32">
        ...
        >
          <div class="flex">
            <button
              v-for="tab in tabs" :key="tab.key" class="flex-1 px-4 py-3 text-center text-sm font-semibold transition-all duration-200 relative"
              :class="activeTab === tab.key
                ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20'
                : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'" @click="activeTab = tab.key"
            >
              <span class="relative z-10">{{ tab.label }}</span>
              <span
                v-if="activeTab === tab.key"
